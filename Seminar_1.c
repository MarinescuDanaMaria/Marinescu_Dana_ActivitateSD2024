...
struct Masina initializareMasina(const char* marca, int an, float cc)
{
	...
	if (marca != NULL)
	{
		m.marca = malloc(strlen(marca) + 1); // cast implicit
		strcpy(m.marca, marca);

	}
	else
	{
		m.marca = (char*)malloc(strlen("N/A") + 1); // cast explicit
		strcpy(m.marca, "N/A");
	}
	return m;
}

void dezalocareMasina(struct Masina* masina) // ( in C nu avem referinte ) punem pointer 
{
	free(masina->marca);
	masina->marca = NULL;
}
